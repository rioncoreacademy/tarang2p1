# ChipCraft Lab — Multi-User Server Setup
...
- Student work is persisted in named Docker volumes (`chipcraft-{username}`)
...
| Field | Value |
|-------|-------|
| Application name | ChipCraft Lab |
...
Select **"ChipCraft Lab — Multi-User Server"** config.
...
```bash
cd /workspaces/chipcraft-lab && docker build -t ubuntu-novnc:latest .
```
...
```bash
cd /workspaces/chipcraft-lab/api && nohup python3 -m uvicorn main:app --host 0.0.0.0 --port 80 > /tmp/api.log 2>&1 &
```
