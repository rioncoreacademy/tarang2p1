#!/bin/bash
# Tarang2_dp1 Lab — Sync encrypted files to GitHub repo
#
# Usage:
#   bash sync_labs.sh <source-folder> <destination-folder>
#
# Examples:
#   bash sync_labs.sh ~/labs/ ~/tarang2-dp1-lab-files/
#   bash sync_labs.sh /home/user/labs/ /home/user/tarang2-dp1-lab-files/
#
# Result:
#   Creates a parent folder named after source inside destination:
#   ~/tarang2-dp1-lab-files/labs/adder.v.enc
#   ~/tarang2-dp1-lab-files/labs/tb/counter.v.enc

if [[ -z "${1:-}" || -z "${2:-}" ]]; then
    echo ""
    echo "Usage  : bash sync_labs.sh <source-folder> <destination-folder>"
    echo ""
    echo "Example: bash sync_labs.sh ~/labs/ ~/tarang2-dp1-lab-files/"
    echo ""
    exit 1
fi
...
if [[ ! -d "$DEST" ]]; then
    ...
    echo "Run  : git clone https://github.com/rioncoreacademy/tarang2-dp1-lab-files.git $DEST"
    exit 1
fi
...
echo ""
echo "========================================"
echo "  Tarang2_dp1 Lab — Sync .enc Files"
echo "========================================"
echo "  From   : $SOURCE/"
echo "  To     : $DEST_FULL/"
echo ""
